...
function Analyticss() {
  ...
  const transactions = [
    { id: "#12345", customer: "John Coe", service: "Plant Care", date: "2024-01-15", amount: 120.00, status: "Completed" },
    { id: "#12346", customer: "Jane Smith", service: "Consultation", date: "2024-01-15", amount: 80.00, status: "Positive" },
    { id: "#12347", customer: "Mike Johnson", service: "Delivery", date: "2024-01-14", amount: 45.00, status: "Processing" },
    { id: "#12348", customer: "Sarah Williams", service: "Maintenance", date: "2024-01-14", amount: 150.00, status: "Completed" },
    { id: "#12349", customer: "Tom Brown", service: "Plant Care", date: "2024-01-13", amount: 85.00, status: "Completed" }
  ];
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
        {metrics.map((metric, index) => (
          <div key={index} className="bg-white rounded-xl shadow-sm p-4 md:p-6 hover:shadow-md transition-shadow duration-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm md:text-base font-medium text-gray-500">{metric.title}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 md:mb-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
          <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-4">Services Distribution</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 md:mb-8">
        {/* User Activity */}
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
          <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-4">User Activity</h2>
          
          <div className="h-64 flex items-end justify-between">
            {userActivity.map((activity, index) => (
              <div key={index} className="flex flex-col items-center flex-1 px-1">
                <div 
                  className="w-8 bg-gradient-to-t from-blue-500 to-blue-400 rounded-t-lg hover:opacity-90 transition-opacity duration-200"
                  style={{ height: `${(activity.value / 200) * 100}%` }}
                ></div>
                <span className="mt-2 text-xs text-gray-500">{activity.day}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Top Cities */}
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
          <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-4">Top Cities</h2>
              ...
              <tbody className="divide-y divide-gray-200">
                {transactions.map((transaction, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-blue-600">{transaction.id}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{transaction.customer}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{transaction.service}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        transaction.status === 'Completed' || transaction.status === 'Positive' 
