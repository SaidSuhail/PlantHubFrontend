    ...
    <div className="relative flex flex-col h-full bg-white shadow-xl z-50 w-72 md:w-64 animate-slide-in-left">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-500 truncate">
                {currentUser?.userEmail || "guest@example.com"}
              </p>
              <button
                onClick={() => setShowProfileModal(true)}
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto">
        <ul className="space-y-1">
          {[
            ...
            { path: "/library", label: "Plant Library", icon: "📚" },
            { path: "/myplants", label: "My Plants", icon: "🌱" },
            { path: "/plans", label: "Plans", icon: "📅" },
            { path: "/orders", label: "Bookings", icon: "📋" }
          ].map((item) => (
            <li key={item.path}>
              <Link
                to={item.path}
      ...
      <div className="px-6 py-3 border-t border-gray-100 bg-gray-50">
        <p className="text-xs text-gray-500">
          <span className="block font-medium text-green-700">PlantCareHub</span>
          Bringing nature to your home since 2023
        </p>
      </div>
    </div>
  </div>
